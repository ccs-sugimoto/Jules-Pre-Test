...
let titleScreenElement;     // タイトル画面のDOM要素
let gameScreenElement;      // ゲーム画面のDOM要素
let startButtonElement;     // ゲーム開始ボタンのDOM要素
...
let score;                  // 現在のスコア
let isGameOver;             // ゲームオーバー状態フラグ (true: ゲームオーバー) -> gameStateで代替検討
let gameState = "title";    // ゲームの状態 (title, playing, gameOver)
...
/**
 * タイトル画面を表示し、ゲーム画面を非表示にする関数。
 */
function showTitleScreen() {
    titleScreenElement.style.display = 'block'; // または 'flex' など、CSSの設計による
    gameScreenElement.style.display = 'none';
    console.log("タイトル画面表示");
}

/**
 * ゲーム画面を表示し、タイトル画面を非表示にする関数。
 */
function showGameScreen() {
    titleScreenElement.style.display = 'none';
    gameScreenElement.style.display = 'flex'; // gameScreenはflexコンテナとして設定したため
    console.log("ゲーム画面表示");
}
...
function spawnNewTetromino() {
    ...
    if (checkCollision(currentTetromino.shape, currentX, currentY, gameBoard)) {
        // isGameOver = true; // gameStateで管理
        gameState = "gameOver";
        console.log("ゲームオーバー！");
        clearInterval(gameIntervalId); // 自動落下処理を停止
        clearInterval(timerIntervalId); // 時間計測処理を停止
        showTitleScreen(); // ゲームオーバーになったらタイトル画面を表示
        // ゲームオーバーメッセージはタイトル画面に任せるか、別途表示方法を検討
        // 必要であれば、ここにゲームオーバー専用のUI更新処理を追加
        alert("ゲームオーバー！\nEnterキーを押してタイトルに戻ります。"); // 仮のアラート
    }
 ...
 */
function gameLoop() {
    if (gameState !== "playing") return; // ゲーム中でない場合は処理中断
...
function startGame() {
    console.log("ゲーム開始");
    gameState = "playing"; // ゲーム状態を"playing"に設定
    ...
    isGameOver = false; // isGameOverフラグも念のためリセット
    score = 0;
...
function handleKeyPress(event) {
    console.log(`キー入力: ${event.key}, 現在のゲーム状態: ${gameState}`);

    if (gameState === "gameOver") {
        // ゲームオーバー時にEnterキーが押されたらタイトル画面に戻る
        if (event.key === 'Enter') {
            console.log("タイトル画面へ戻る");
            gameState = "title";
            showTitleScreen();
        }
        return;
    ...
    if (gameState !== "playing") return; // プレイ中でなければキー操作を無視
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    timeElement = document.getElementById('timeArea');
    ...
    startButtonElement.addEventListener('click', () => {
        ...
        // gameState = "playing"; // startGame内で設定
        showGameScreen();
        startGame();
    });
});
